...
function setupRecruit_(ss) {
  const sh = getOrCreateSheet_(ss, 'シフト・事務募集');
  header_(sh, ['日時', '内容', '残り人数', '掲載終了日'], [160, 420, 90, 120]);
  noteRow_(sh, '日時は自由記述OK（例: 2026/07/13、毎週金曜 17時〜）。残り人数は「あと◯名」と赤字表示。');
  dateRule_(sh, 'D2:D500');
  sample_(sh, [['毎週金曜 17時〜', '受付シフト', 1, today_(14)]]);
}
